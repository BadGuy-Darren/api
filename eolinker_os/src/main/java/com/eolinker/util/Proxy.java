...
public class Proxy
{
	private org.slf4j.Logger logger = LoggerFactory.getLogger(Proxy.class);
	//请求远程服务
			...
			List<Map<String, String>> headerList, List<Map<String, String>> paramList)
	{
		...
		try
		{
			 ...
			 HttpEntity<?> requestEntity = new HttpEntity<>(testParams, requestHeaders);
		...
		catch (Exception e)
		{
			...
			logger.warn("Proxy访问api出错：{}", e.getMessage());
			e.printStackTrace();
			 endTime = System.currentTimeMillis();
			 deny = endTime - startTime;
			 Map<String, Object> testResult = new HashMap<String, Object>();
	 ...
	 * @return
	 */
	public Map<String, Object> newProxyToDesURL(String method, String completeURL,
											 List<Map<String, String>> headerList, Map<String, Object> body)
	{
		// TODO Auto-generated method stub
		Map<String, Object> result = new HashMap<String, Object>();
		Date nowTime = new Date();
		...
		try
		{
			...
			testResult.put("headers", responseHeaders);
			testResult.put("body", responseBody);
			result.put("testTime", testTime);
			result.put("testHttpCode", statusCode);
			result.put("testResult", testResult);
			result.put("testDeny", deny);
