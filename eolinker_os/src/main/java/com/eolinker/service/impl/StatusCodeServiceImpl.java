...
public class StatusCodeServiceImpl implements StatusCodeService
{
	 ...
	 */
	@Override
	public boolean checkCode(String code) throws RuntimeException {
		// TODO Auto-generated method stub
		List<StatusCode> list = statusCodeMapper.getAllCodeListByCode(code);
	 ...
	 */
	@Override
	public boolean checkCode(String code, Integer codeID) throws RuntimeException {
		// TODO Auto-generated method stub
		List<StatusCode> list = statusCodeMapper.getAllCodeListByCode(code);
		boolean result = false;
		if (list != null && list.size() > 0) {
			for (StatusCode statusCode : list) {
				if (codeID == statusCode.getCodeID()) {
					result = true;
					break;
				}
			}
