...
public class StatusCodeController {
	...
	public Map<String, Object> addCode(StatusCode statusCode, String codeDesc) {
		...
		try {
			if (statusCode.getGroupID() == null || !String.valueOf(statusCode.getGroupID()).matches("^[0-9]{1,11}$"))
				...
				// 校验状态码是否已存在，如果存在则返回状态码：190005
				boolean checkCode = this.statusCodeService.checkCode(statusCode.getCode());
				if (!checkCode) {
	...
	public Map<String, Object> checkCode(@RequestParam("code") String code) {
		...
		try {
			boolean result = this.statusCodeService.checkCode(code);
			map.put("statusCode", (result) ? "000000" : "190005");
		} catch (RuntimeException e) {
			e.printStackTrace();
			map.put("statusCode", "190000");
			return map;
	...
	public Map<String, Object> editCode(StatusCode statusCode, String codeDesc) {
		...
		try {
			if (statusCode.getCodeID() == null || !String.valueOf(statusCode.getCodeID()).matches("^[0-9]{1,11}$"))
				...
				// 校验状态码是否已存在，如果存在则返回状态码：190005
				boolean checkCode = this.statusCodeService.checkCode(statusCode.getCode(), statusCode.getCodeID());
				if (!checkCode) {
					map.put("statusCode", "190005");
					return map;
				}
	...
	public Map<String, Object> addStatusCodeByExcel(HttpServletRequest request, HttpServletResponse response,
			Integer groupID, @RequestParam("excel") MultipartFile file) {
		Map<String, Object> map = new HashMap<String, Object>();
		if (file == null) {
			map.put("statusCode", "190005");
			return map;
