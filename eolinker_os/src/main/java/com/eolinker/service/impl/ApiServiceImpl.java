...
public class ApiServiceImpl implements ApiService {
	...
	public Integer addApi(Api api, String apiHeader, String apiRequestParam, String apiResultParam) {
		...
		if (api.getApiMethodType() == null) {
			api.setApiMethodType("0");
		}
		if (api.getApiMethod() == null) {
			api.setApiMethod("");
		}
		Date date = new Date();
	 ...
	 */
	@Override
	public boolean editApi(Api api, String apiHeader, String apiRequestParam, String apiResultParam,
			String updateDesc) {
		// TODO Auto-generated method stub
		if (api.getApiSuccessMock() == null) {
			api.setApiSuccessMock("");
	...
	public List<Map<String, Object>> getApiList(Integer projectID, Integer groupID, Integer orderBy, Integer asc) {
		...
		if (childGroupList != null && !childGroupList.isEmpty()) {
			for (Map<String, Object> group : childGroupList) {
				...
				List<Map<String, Object>> childGroupList1 = apiGroupMapper.getChildGroupList(projectID,
	...
	public Api getApiInfo(Integer projectID, Integer apiID) {
		// TODO Auto-generated method stub
		return apiMapper.getApiInfo(projectID, apiID);
	}
	 ...
	 */
	@Override
	public boolean checkApi(Api api) {
		// TODO Auto-generated method stub
		List<Api> list = null;
		//apiMethodType为1时，URL不作为查询条件，为0时查询条件为URL和apiname 
		if ("0".equals(api.getApiMethodType())) {
			list = apiMapper.getAllApiForCheck(api.getApiID(), api.getApiName(), api.getApiURI());
		} else {
			list = apiMapper.getAllApiForCheck(api.getApiID(), api.getApiName(), null);
		}
